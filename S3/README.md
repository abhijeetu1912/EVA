...
## Model Architecture
1. convolution (16 out channels) -> relu -> batch normalization
2. convolution (16 out channels) -> relu -> batch normalization -> max pooling -> dropout
3. convolution (16 out channels) -> relu -> batch normalization -> dropout
4. convolution (16 out channels) -> relu -> batch normalization -> dropout
5. convolution (64 out channels) -> relu -> batch normalization -> global average pooling -> dropout
...
I didn't apply dropout after 1st convolution layer to make sure I don't lose out on any initial information. Also kept higher number of channels in last convolution layer compared to initial convolutions to make sure enough features are preserved after global average pooling. As model was itself not overfitting much, I used a lower value of dropout. I used a FC layer before output layer to further filter out important features generated by global average pooling. I changed the batch size as 32 to make sure desired performance accuracy is achived below 20 epochs. Lower batch size means more number of iterations for weight updates, hence faster improvement in performance depedning upon data and usecase.
